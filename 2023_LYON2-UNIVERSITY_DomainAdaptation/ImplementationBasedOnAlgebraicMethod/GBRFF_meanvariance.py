...
class gbrff_da_meanvariance(object):
    def __init__(self, gamma=0.1, Lambda_0=0, Lambda_b=0, Lambda_omega=0, T=100, randomState=np.random):
        self.T = T
    ...
    def loss_grad(self, omega):
        ...
        return ((1/self.n_s)*np.sum(v0) + self.Lambda_0*np.sum(omega**2) + self.Lambda_omega*((
                                    (1/self.n_s)*np.sum(self.yTildePred)-
                                    (1/self.n_t)*np.sum(self.yTildePred_t))**2 + 
                                              ((1/self.n_s)*np.sum(self.yTildePred**2) -
                                              (1/self.n_t)*np.sum(self.yTildePred_t**2))**2),
                (1/self.n_s)*(self.yTilde*v0*np.sin(dots)).dot(
                    self.X) + self.Lambda_0*2*np.sum(omega) + self.Lambda_omega*2*(
                                (1/self.n_s)*np.sum(self.yTildePred) -
                                (1/self.n_t)*np.sum(self.yTildePred_t)) * (
                                (1/self.n_s)*np.sin(dots).dot(self.X) -
                                (1/self.n_t)*np.sin(dott).dot(self.Xt)) + self.Lambda_omega*2*(
                                            (1/self.n_s)*np.sum(self.yTildePred**2) -
                                            (1/self.n_t)*np.sum(self.yTildePred_t**2)) * (
                                            (2/self.n_s)*(np.cos(dots)*np.sin(dots)).dot(self.X) -
                                            (2/self.n_t)*(np.cos(dott)*np.sin(dott)).dot(self.Xt)))
    ...
    def fit(self, y, Xs, Xt):
        ...
        for t in range(self.T):
            ...
            self.b = pi2*fminbound(lambda n: (1/self.n_s)*np.sum(np.exp(
                       self.yTildeN*np.cos(pi2*n - wx_s))) + self.Lambda_b*((
                                 (1/self.n_s)*np.sum(np.cos(pi2*n - wx_s)) -
                                 (1/self.n_t)*np.sum(np.cos(pi2*n - wx_t)))**2+(
                                           (1/self.n_s)*np.sum(np.cos(pi2*n - wx_s)**2) -
                                           (1/self.n_t)*np.sum(np.cos(pi2*n - wx_t)**2))**2),
                                   -0.5, 0.5, xtol=1e-2)
            self.xts[t] = self.b
